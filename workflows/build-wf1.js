...
code(
  ...
  [
    ...
    "const calRounds = ($('Jolpica Calendar').first().json.MRData.RaceTable.Races || []).map((r) => { const rd = parseInt(r.round); const dt = new Date(r.date); return { round: rd, name: r.raceName.replace(/ Grand Prix.*$/, ''), dateLabel: (dt.getUTCMonth() + 1) + '월 ' + dt.getUTCDate() + '일', flag: FLAG[r.Circuit.Location.country] || null, ...(winners[rd] || {}) }; });",
    "const cards = [",
