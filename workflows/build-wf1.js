...
code(
  ...
  [
    ...
    "  // 신설 서킷(작년 이 서킷 레이스 없음)엔 작년 컴파운드가 존재하지 않는다 — '작년: 해당 없음' 으로 명시한다 (2026-09-10 확정).",
    "  // 2026 마드리드에서 LLM 이 GP 이름만 같은 바르셀로나 값을 가져온 뒤 생긴 규칙이다.",
    "  { type: 'tires', template: 'tires', needsLlm: ['compounds'], data: { raceName: meta.raceName, body: (() => { const last = meta.lastYear ? rs.tiresLastYear : '해당 없음'; return ((last ? '작년: ' + last : '') + (rs.tiresThisYear ? '\\n올해: ' + rs.tiresThisYear : (last ? '\\n올해: 발표 대기' : ''))).trim() || null; })() } },",
...
code(
  ...
  [
    ...
    "// 세션 결과는 전원을 노출한다. 상위 10명만 자르면 잘린 것처럼 보인다.",
    "// session-result 템플릿은 한 열이라 22명이 안 들어가므로 2열인 result-full 을 쓴다.",
    "// rows 형태가 달라 매핑이 필요하다: time → metric, dnf → out",
    "const full = (title, rows) => ({",
    "  type: 'session-result', template: 'result-full',",
    "  data: { title, subtitle: meta.raceName, note: '공식 결과',",
    "    rows: rows.map((x) => ({ pos: x.pos, code: x.code, name: x.name, team: x.team, metric: x.time, out: !!x.dnf })) },",
    "});",
    "let cards;",
...
code(
  ...
  [
    ...
    "// 스틴트는 순위표가 아니라 랩 타임라인이라 전원을 넣으면 행이 26px까지 눌려 구간 라벨이 사라진다.",
    "// 상위 10명으로 제한하고, 그 범위를 카드에 배지로 명시한다 (잘린 것처럼 보이지 않게).",
    "const stintRows = rows.slice(0, 10).map((r) => ({ pos: r.pos, code: r.code, name: r.name, stints: codeToStints[r.code] || [] }));",
    ...
    "// 국기는 개최국 기준 (Jolpica Circuit.Location.country → assets/flags/<iso>.png). 이름은 fullName 규칙을 따르도록 풀네임을 넘긴다.",
    "// 2026 중동 정세로 카타르·아부다비가 유럽으로 옮겨질 수 있어 후보 개최국(포르투갈·터키·독일·프랑스)과 원래 개최국(바레인·사우디)도 미리 매핑했다. 매핑에 없는 나라는 국기 칸만 빈다.",
    "const FLAG = { Australia: 'au', China: 'cn', Japan: 'jp', USA: 'us', 'United States': 'us', Canada: 'ca', Monaco: 'mc', Spain: 'es', Austria: 'at', UK: 'gb', Belgium: 'be', Hungary: 'hu', Netherlands: 'nl', Italy: 'it', Azerbaijan: 'az', Malaysia: 'my', Singapore: 'sg', Mexico: 'mx', Brazil: 'br', Qatar: 'qa', UAE: 'ae', Portugal: 'pt', Turkey: 'tr', Germany: 'de', France: 'fr', Bahrain: 'bh', 'Saudi Arabia': 'sa' };",
    "const winners = {}; for (const w of ($('RC Winners').first().json.MRData.RaceTable.Races || [])) { const r0 = (w.Results || [])[0]; if (r0) winners[parseInt(w.round)] = { winner: r0.Driver.givenName + ' ' + r0.Driver.familyName, team: r0.Constructor.name }; }",
    "// 'Bahrain Grand Prix in Malaysia' 처럼 이름의 나라와 개최국이 다르면 국기는 개최국, 이름은 GP, 날짜 줄에 '○○ 개최'를 덧붙인다.",
    "const KR = { Australia: '호주', China: '중국', Japan: '일본', USA: '미국', 'United States': '미국', Canada: '캐나다', Monaco: '모나코', Spain: '스페인', Austria: '오스트리아', UK: '영국', Belgium: '벨기에', Hungary: '헝가리', Netherlands: '네덜란드', Italy: '이탈리아', Azerbaijan: '아제르바이잔', Malaysia: '말레이시아', Singapore: '싱가포르', Mexico: '멕시코', Brazil: '브라질', Qatar: '카타르', UAE: 'UAE', Portugal: '포르투갈', Turkey: '터키', Germany: '독일', France: '프랑스', Bahrain: '바레인', 'Saudi Arabia': '사우디' };",
    "const calRounds = ($('Jolpica Calendar').first().json.MRData.RaceTable.Races || []).map((r) => { const rd = parseInt(r.round); const dt = new Date(r.date); const m = r.raceName.match(/^(.*?) Grand Prix(?: in (.+))?$/); const venue = m && m[2] ? m[2].trim() : null; return { round: rd, name: m ? m[1] : r.raceName, dateLabel: (dt.getUTCMonth() + 1) + '월 ' + dt.getUTCDate() + '일', venue: venue ? (KR[venue] || venue) : null, flag: FLAG[r.Circuit.Location.country] || null, ...(winners[rd] || {}) }; });",
    "const cards = [",
