...
n(
  ...
  'code',
  {
    jsCode: [
      ...
      "// URL 을 넘기면 텔레그램 서버가 우리 도메인에서 이미지를 받아와야 한다. 2026-09-10 텔레그램 쪽 지연으로",
      "// 84초간 요청이 nginx 에 닿지 않아 'failed to get HTTP URL content' 로 전부 실패했다. 서버 디스크의",
      "// 파일을 바이너리로 직접 업로드하면 외부에서 우리 서버에 접근할 필요가 없어진다.",
      "const fs = require('fs');",
      "const toPath = (u) => u.replace(/^https?:\\/\\/[^/]+\\/hooni-cards\\//, '/data/cards/');",
      "const items = p.urls.map((u, i) => ({ url: u, caption: '카드 ' + (i + 1) + '/' + p.urls.length + mark(i + 1) }));",
      "if (p.storyUrl) items.push({ url: p.storyUrl, caption: '📱 인스타 스토리 (9:16)' });",
      "const out = [];",
      "for (const it of items) {",
      "  const path = toPath(it.url);",
      "  const buf = fs.readFileSync(path);",
      "  out.push({ json: { photoUrl: it.url + '?v=' + p.revision, caption: it.caption }, binary: { photo: await this.helpers.prepareBinaryData(buf, path.split('/').pop(), 'image/png') } });",
      "}",
      "return out;",
    ].join('\n'),
  },
...
n(
  'Send Cards',
  'telegram',
  { operation: 'sendPhoto', chatId: TG_CHAT_ID, binaryData: true, binaryPropertyName: 'photo', additionalFields: { caption: '={{ $json.caption }}' } },
  [600, 0],
  1.2,
