...
| **WF-8** ig-token-refresh | 매일 08:30 인스타그램 장기 토큰을 갱신해 서버 파일에 덮어쓴다. 실패하면 텔레그램 알림 |
...
노드 구성을 세어 보면 Code 64개, HTTP Request 52개로 전체의 대부분이다. 로직은 전부 JS이고, n8n은 그것을 **실행하고, 기다리고, 깨우는 껍데기**다. n8n이 아니면 불가능한 것은 없다 — `workflows/local-render.js` 가 WF-2의 jsCode를 JSON에서 꺼내 로컬 Chrome으로 돌리는 것이 그 증거다. 그럼에도 n8n을 택한 이유는 세 가지다.

**① 승인 대기 → 재개를 공짜로 얻는다.** 이 파이프라인의 핵심은 "카드를 보내고, 사람이 볼 때까지 기다리고, 답이 오면 그 자리에서 이어간다"이다. WF-3의 Wait 노드는 실행 상태를 통째로 얼려두고, 텔레그램 버튼이 `resumeUrl` 을 치면 24시간 안에 이어서 돈다. 이걸 직접 만들면 대기 상태 저장소, 웹훅을 받는 상시 서버, 만료 처리 스케줄러, 프로세스 재시작 후 복구까지 넷이 필요하다. **노드 하나가 서비스 하나로 바뀐다.** n8n의 가장 큰 존재 이유다.

**② 실행 이력을 노드 단위로 되짚을 수 있다.** "모델이 200 OK로 `null` 을 돌려줬다", "HTTP 노드가 60번 반복 호출됐다" 같은 문제는 실행 화면에서 노드별 입출력을 눌러 보며 찾았다. 주말에만 돌고 실패가 드문 파이프라인은 "그때 뭐가 들어왔었지"를 볼 수 있어야 고칠 수 있다. 직접 짜면 구조화 로깅과 보관을 따로 만들어야 하고, 그걸 만들기 전까지의 실패는 그냥 사라진다.

**③ 비밀키가 저장소에 들어갈 경로가 없다.** 텔레그램·Gemini·browserless 토큰은 n8n Credentials 안에 있고, 워크플로 JSON에는 id만 들어간다. 인스타그램 토큰만은 60일마다 값이 바뀌어 Credentials 로 따라갈 수 없어서 서버 파일(`/data/hooni_speed/ig-token.json`)에 두고 WF-8 이 갱신한다. 역시 저장소 밖이다. 이 저장소는 공개돼 있으므로 `.env` 와 `.gitignore` 에 기대는 것보다 안전하다.

비용은 알고 택했다. 상시 메모리를 수백 MB 먹고, 배포가 `import → --active=true → restart` 3단계이며, 루프가 없어 재시도를 노드로 펼쳐야 하고, 노드 단위 테스트가 어렵다. 그 대가로 얻는 것이 ①·②·③이고, 특히 ①은 혼자 운영하는 주 5회 파이프라인에서 직접 만들어 유지할 만한 것이 아니었다.
